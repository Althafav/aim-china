window.cfields = {
  ...
  374: "aim_proposed_event_title",
  375: "aim_proposed_side_event_details",
  376: "speaker_call_bio_link",
  377: "speaker_call_details",
  378: "aim_investor_details",
  6: "message",
  328: "mainsource",
  329: "subsource",
  38: "forms_submitted",
...
(function () {
  ...
  async function form_submit(e) {
    ...
    if (validate_form()) {
      ...
      const bioInput = document.getElementById("bioAttachment");

      if (bioInput && bioInput.files && bioInput.files.length > 0) {
        const fileData = new FormData();
        fileData.append("file", bioInput.files[0]);

        try {
          const resp = await fetch(
            "https://media.aimcongress.com/api/Upload/Asset",
            {
              method: "POST",
              body: fileData,
            }
          ...
          bioUrl = uploadJson.FileUrl || "";
        } catch (err) {
          console.error("Bio upload error:", err);
        }
      }

      const speakerData = $('textarea[name="field[377]"]').val() || "";
